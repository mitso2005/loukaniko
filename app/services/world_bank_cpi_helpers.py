...
DB_PATH = Path("app/db/cpi_data.db")


def get_latest_cpi(country_code: str, db_path: str = None) -> Optional[float]:
    """
    Get the most recent CPI value for a country.
    
    Args:
        country_code: ISO 3-letter country code (e.g., 'AUS', 'USA')
        db_path: Path to database (optional, uses default if not provided)
    
    Returns:
        Latest CPI value, or None if not found
